...
void message_display( float temperature) {
    clean_display_oled();
    char sensor_temp[50];
    sprintf(sensor_temp, "%.2f°C", temperature);
    show_message("Temperatura", 20, 10, true);
    show_message("Interna da MCU", 10, 25, true);
    show_message(sensor_temp, 33, 45, false);
}
...
int main()
{
    ...
    while (true) {
        ...
        message_display(temperature_celsius);
